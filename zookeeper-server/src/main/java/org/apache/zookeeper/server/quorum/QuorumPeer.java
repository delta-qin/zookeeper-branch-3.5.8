...
public class QuorumPeer extends ZooKeeperThread implements QuorumStats.Provider {
    ...
    public synchronized void start() {
        ...
        loadDataBase();
        // 刚刚初始化完，现在开始启动
        // 设置了参数的话，这里启动的就是 netty server，绑定客户端端口
        startServerCnxnFactory();
        try {
            // 8080 端口，内嵌 jetty server ，查看机器状态的相关信息
            adminServer.start();
        } catch (AdminServerException e) {
            LOG.warn("Problem starting AdminServer", e);
            System.out.println(e);
        }
        // 初始化 leader 选举的投票环境，包括绑定收发端口，
        // 初始化收发队列，初始化收发 worker
        startLeaderElection();
        // 最核心选举链路：启动集群选举 leader 的线程
        // 就直接看当前类的 run 方法，当前类就是 Thread的子类
        super.start();
    }
    ...
    protected Election createElectionAlgorithm(int electionAlgorithm){
        ...
        case 3:
            ...
            if(listener != null){
                // 这里都是创建一些线程和队列用来监听和收发消息

                // 启动分线程，启动 BIO serverSocket,监听端口，接收选票
                listener.start();
                // 选举算法。初始化发送和接收的队列
                FastLeaderElection fle = new FastLeaderElection(this, qcm);
                // 启动两个子线程，发送和接收的工作者，和队列联合起来。快速选举算法的实现
                fle.start();
                le = fle;
            } else {
